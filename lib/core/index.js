...
function core() {
	...
	self.userLogin = function(username, password, ip) {
		if (user.authenticate(username, password, ip)) {
			logit.log('valid login', username + ' - from ' + ip, 'green');
			return true;
		} else {
			logit.log('invalid login', username + ':' + password + ' - from ' + ip, 'red');
			return false;
		}
	}
